...
Use `memory_store` with a positive integer `ttl` in seconds for scratch state such as “mid-task: step 3 of 5 done.” Prefer the `working` namespace, but only after the operator explicitly grants that namespace to the API key. Do not use TTL for decisions, preferences, lessons, or other facts that future sessions should retain. Expiring writes remain distinct (`dedupe:true` is invalid), return their database-computed `expires_at`, and disappear exactly at that instant even if maintenance cleanup is delayed. Update or replace scratch state using an exact idempotency key when retry identity is needed. Document storage does not support TTL.
