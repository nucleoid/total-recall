...
export async function finalizeMemoryValidity(client: ValidityFinalizerClient): Promise<void> {
  ...
  if (Number(row?.missing ?? 0) > 0 || Number(row?.invalid ?? 0) > 0 || Number(row?.mismatched ?? 0) > 0) {
    throw new Error(
...
async function ensureConcurrentIndex(
  ...
  const load = () => client.query<{ valid: boolean; definition: string }>(`
    SELECT i.indisvalid AS valid, pg_get_indexdef(index_relation.oid) AS definition
  ...
  if (existing?.valid && !indexDefinitionMatches(existing.definition, expectedFragments)) {
  ...
  if (!after?.valid || !indexDefinitionMatches(after.definition, expectedFragments)) {
    throw new Error(`Validity index ${name} is missing, invalid, or has an unexpected definition`);
  }
}
