...
test('valid_at is a strict offset-aware instant and migration preserves staged backfill', () => {
  ...
  const migration = readFileSync(new URL('../migrations/025_memory_kind_and_validity.sql', import.meta.url), 'utf8');
  ...
  assert.match(migration, /memories_validity_interval_check/);
