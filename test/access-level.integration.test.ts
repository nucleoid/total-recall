...
assertIncludes('src/tools/register.ts', 'listAgents(auth)');
assertIncludes('src/server.ts', 'listAgents(auth)');
assertIncludes('src/tools/store.ts', 'ensureAccessLevelAllowed');
assert.equal(storeDocumentSchema.parse({
  title: 'classified doc',
  content: 'secret content',
  access_level: 'secret',
}).access_level, 'secret');
assertIncludes('src/tools/store-document.ts', 'ensureAccessLevelAllowed');
assertIncludes('src/tools/store-document.ts', 'params.access_level');
