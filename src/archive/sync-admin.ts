...
export async function provisionArchiveSync(pool:Pool,archiveId:string,readerIds:string[]){
  ...
  try{
    ...
    const existing=(await client.query('select id from api_keys where name=$1 order by created_at',[name])).rows;
    if(existing.length>1)throw new Error('archive_sync.ambiguous_identity');
    let keyId:string;
    if(existing.length){keyId=existing[0].id;await syncAuth(client,keyId);}
