...
export async function memoryList(
  ...
  const res = await queryScoped(
    scope,
    `SELECT m.id, m.content, m.source, m.namespace, m.tags, m.metadata, m.document_id, m.chunk_index,
       m.created_at, m.updated_at, m.supersedes_id, m.superseded_at, m.revision,
